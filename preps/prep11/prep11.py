...
def merge3(lst1: List, lst2: List, lst3: List) -> List:
    ...
    while index1 < len(lst1) and index2 < len(lst2) and index3 < len(lst3):
        if lst1[index1] <= lst2[index2] <= lst3[index3] or \
                lst1[index1] <= lst3[index3] <= lst2[index2]:
            merged.append(lst1[index1])
            index1 += 1
        elif lst2[index2] <= lst1[index1] <= lst3[index3] or \
                lst2[index2] <= lst3[index1] <= lst1[index3]:
            merged.append(lst2[index2])
            index2 += 1
        else:
    ...
    end_list1 = lst1[index1:]
    end_list2 = lst2[index2:]
    end_list3 = lst3[index3:]
    if len(end_list1) != 0 and len(end_list2) != 0 and len(end_list3) == 0:
        if end_list1[0] <= end_list2[0]:
            return merged + end_list1 + end_list2
        return merged + end_list2 + end_list1
    if len(end_list1) != 0 and len(end_list3) != 0 and len(end_list2) == 0:
        if end_list1[0] <= end_list3[0]:
            return merged + end_list1 + end_list3
        return merged + end_list3 + end_list1
    if len(end_list2) != 0 and len(end_list3) != 0 and len(end_list1) == 0:
        if end_list2[0] <= end_list3[0]:
            return merged + end_list2 + end_list3
        return merged + end_list3 + end_list2
...
def kth_smallest(lst: List, k: int) -> Any:
    ...
    if len(smaller) == k:
        ...
        return kth_smallest(bigger, k - len(bigger) - 1)
