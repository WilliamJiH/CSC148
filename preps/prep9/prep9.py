...
class BinarySearchTree:
    ...
    def maximum(self) -> Optional[int]:
        ...
        if self.is_empty():
            return None
        elif not self._right._root:
            return self._root
        else:
            curr = self._right
            while curr._right._root:
                curr = curr._right
            return curr._root
    ...
    def count(self, item: Any) -> int:
        ...
        if self._root == item:
            acc += 1
        acc += self._left.count(item)
        acc += self._right.count(item)
        return acc
    ...
    def smaller(self, item: Any) -> List:
        ...
        if self.is_empty():
            ...
            res = []
            res.extend(self._left.smaller(item))
            res.append(self._root)
            res.extend(self._right.smaller(item))
            return res
        else:
            res = []
            res.extend(self._left.smaller(item))
...
if __name__ == '__main__':
    ...
    python_ta.check_all(config={"disable": ["E9959"]})
